...
AGENTS = {
    ...
    "BruteForceAgent": BruteForceAgent,
    "PoolMasterAgent": PoolMasterAgent
}
from poolagent.pool import PoolGame
from poolagent.utils import blur_shot, SKILL_LEVELS, TwoPlayerState
...
class Matchup(Task):
    def __init__(self, description, agent_one, agent_two, num_ball_level, num_trials=10):
...
class SequentialExperimentManager:
    def __init__(self, experiment_name, gpu_ids, gpu_size=40):
        ...
        # Setup logging
        self.log_dir = os.path.join(ROOT_DIR, "experiments", "experiment_one", "num_ball_logs", self.timestamp)
    ...
    def load_previous_results(self):
        results_folders = ROOT_DIR + "/experiments/experiment_one/num_ball_results"
    ...
    def run_game(self, matchup, game_num, num_ball_level):
        ...
        matchup_dir = os.path.join(ROOT_DIR, "experiments", "experiment_one", "num_ball_logs", self.timestamp, str(num_ball_level), "tasks", matchup_key)
        os.makedirs(matchup_dir, exist_ok=True)

        m_agent_one = matchup.agent_one
        m_agent_two = matchup.agent_two
        agent_one = AGENTS[m_agent_one.name](target_balls=player_one_balls)
        agent_two = AGENTS[m_agent_two.name](target_balls=player_two_balls)
        ...
        self.env.reset()
        self.env.target_balls = target_balls
        state = TwoPlayerState(num_ball_level=num_ball_level)
        ...
        while not self.env.check_win():
            ...
            for retry in range(max_retries):
                try:
                    ...
                    self.env.take_shot(current_settings['current_player'], blur_shot(shot, skill=SKILL_LEVELS.BASELINE))
    ...
    def run_matchup(self, matchup):
        ...
        num_ball_level = matchup.num_ball_level
        ...
        matchup_dir = f"{self.log_dir}/{num_ball_level}/tasks/{matchup.description}"
        os.makedirs(matchup_dir, exist_ok=True)
        ...
        for game_num in range(n_games):
            ...
            try:
                game_result = self.run_game(matchup, game_num, num_ball_level)
...
def run_experiment(experiments_config, args):
    ...
    weave.init(f"ExperimentOne-num-balls-{exp_description}") 
    ...
    total_matchups = len(NUM_BALL_LEVELS) * (len(agents)**2 - len(agents))
    ...
    def completed_matchup_trials(results, num_ball_level, key):
        results = results.get(num_ball_level, {})
    ...
    matchups = []
    index = 0
    for num_ball_level in NUM_BALL_LEVELS:
        for i, a1 in enumerate(agents):
            for j, a2 in enumerate(agents):
                if i != j:
                    if index >= start_idx and index < end_idx:
                        key = gen_key(a1, a2)
                        matchup_trials = num_trials - completed_matchup_trials(manager.results, num_ball_level, key)
                        if matchup_trials > 0:
                            matchup = Matchup(key, a1, a2, num_ball_level, num_trials=matchup_trials)
