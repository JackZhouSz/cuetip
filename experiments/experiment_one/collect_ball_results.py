...
def process_ball_level(num_ball_path, all_results):    
    pattern = f'{num_ball_path}/tasks/*/results.json'
...
def combine_results():
    os.makedirs('num_ball_results', exist_ok=True)
    os.makedirs('num_ball_logs', exist_ok=True)

    # Define num ball levels
    num_ball_levels = [2, 4, 5, 6]
    combined_results = defaultdict(dict)
    
    # Process each num ball level
    paths = glob('num_ball_logs/*')
    for path in paths:
        for ball_level in num_ball_levels:
            num_ball_path = f'{path}/{ball_level}'
            print(f"\nProcessing {num_ball_path}...")
            if os.path.exists(num_ball_path):
                print(f"Found {ball_level} data at {num_ball_path}")
                combined_results[ball_level] = process_ball_level(num_ball_path, combined_results[ball_level])
    
    # Save combined results
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    output_file = f'num_ball_results/{timestamp}.json'
    try:
        ...
        print("\nSummary:")
        for ball_level, num_ball_data in combined_results.items():
            print(f"\n{str(ball_level).upper()}:")
            for task, data in num_ball_data.items():
                print(f"\n{task}:")
