...
class Matchup(Task):
    def __init__(self, description, agent_one, agent_two, num_trials=10):
...
class SequentialExperimentManager:
    def __init__(self, experiment_name, gpu_ids, gpu_size=40, eidf_save_intermediate_results=False):
        ...
        # Setup logging
        self.log_dir = os.path.join(ROOT_DIR, "experiments", "experiment_one", "logs", self.timestamp)
    ...
    def load_previous_results(self):
        results_folders = ROOT_DIR + "/experiments/experiment_one/results"
    ...
    def get_agent_shot(self, env, state, player, current_settings, logger, lm):
        ...
        elif isinstance(player, BruteForceAgent):
            return self.bot_shot(env, state, player)
        elif isinstance(player, PoolMasterAgent):
            return self.bot_shot(env, state, player)
        ...
        while not self.env.check_win():
            ...
            for retry in range(max_retries):
                try:
                    ...
                    shot = self.get_agent_shot(self.env, state, player, current_settings, self.logger, current_llm)
                    end_time = time.time()
...
def run_experiment(experiments_config, args):
    ...
    manager = SequentialExperimentManager("experiment_one", gpu_ids, gpu_size, eidf_save_intermediate_results=eidf)
    ...
    weave.init(f"ExperimentOne-{exp_description}") 
    ...
    total_matchups = len(agents)**2 - len(agents)
    ...
    def completed_matchup_trials(results, key):
        ...
        for j, a2 in enumerate(agents):
            if i != j:
                if index >= start_idx and index < end_idx:
                    key = gen_key(a1, a2)
                    matchup_trials = num_trials - completed_matchup_trials(manager.results, key)
                    if matchup_trials > 0:
                        matchup = Matchup(key, a1, a2, num_trials=matchup_trials)
                        matchups.append(matchup)
                index += 1

    print(f"Running {len(matchups)}/{len(agents)**2 - len(agents)} matchups")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--num_trials", type=int, default=10, help="Number of trials to run for each matchup")
